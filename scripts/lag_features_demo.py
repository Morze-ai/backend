...
def demo_rolling_aggregates():
    ...
    rolling_cols = [
        col
        for col in df_with_rolling.columns
        if "rainfall_mm_" in col and "_" in col.split("rainfall_mm_")[1]
    ]
    ...
    print(df_with_rolling[["timestamp", "rainfall_mm", *rolling_cols[4:8]]].iloc[5:11])
...
def demo_seasonal_features():
    ...
    for label, idx in examples:
        ...
        print(f"\n{label}:")
        print(
            f"  month: {row['month']}, season: {row['season']}, is_growing_season: {row['is_growing_season']}"
        )
...
def demo_warmup_period():
    ...
    print(
        f"  - First row has lags: {not df_with_lags.iloc[0][['rainfall_mm_lag_1h', 'rainfall_mm_lag_72h']].isna().all()}"
    )
    ...
    print(
        f"  - First row now has complete lags: {not df_clean.iloc[0][['rainfall_mm_lag_1h', 'rainfall_mm_lag_72h']].isna().all()}"
    )
...
def show_commands():
    ...
    commands = [
        ...
        ("Run tests", "uv run pytest tests/test_feature_engineering.py -v"),
        ("Run all tests", "uv run pytest tests/ -q"),
        ("View lag features guide", "cat docs/LAG_FEATURES_GUIDE.md"),
        ("Run this demo", "cd scripts && python lag_features_demo.py"),
    ]
